...
class LLTF:
    ...
    def set_wave(self, wavelength):
        ...
        #pe_set_harmonic_filter_enabled = self._library.PE_SetHarmonicFilterEnabled
        #pe_set_harmonic_filter_enabled.argtypes = [ct.c_void_p, ct.c_int]
        #pe_set_harmonic_filter_enabled.restype = PE_STATUS
        ...
        try:
            ...
            if check_wave == wavelength:
                ...
                #FWHM / Bandwidth is 5.0 nm
                resolution = 5.0/8
                if new_wave > wavelength+resolution or new_wave < wavelength-resolution:
    ...
    def _harmonic_filter(self):
        ...
        """
        pe_has_harmonic_filter = self._library.PE_HasHarmonicFilter
        pe_has_harmonic_filter.argtypes = [ct.c_void_p]
        pe_has_harmonic_filter.restype = ct.c_int

        pe_get_harmonic_filter_enabled = self._library.pe_GetHarmonicFilterEnabled
        pe_get_harmonic_filter_enabled.argtypes = [ct.c_void_p, ct.POINTER(ct.c_int)]
        pe_get_harmonic_filter_enabled.restype = PE_STATUS

        avail = pe_has_harmonic_filter(self._handle)
        enable_i = ct.c_int()
        harmonic_status = pe_get_harmonic_filter_enabled(self._handle, ct.byref(enable_i))
